def select_books_titles_and_years_in_first_series_order_by_year
  "SELECT title, year FROM books WHERE series_id = (SELECT MIN(series_id) FROM books) ORDER BY year ASC"
end
...
def select_value_and_count_of_most_prolific_species
  "SELECT species, COUNT(species) AS counted FROM characters GROUP BY species ORDER BY counted DESC LIMIT 1"
end
...
def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, COUNT(*) as total_books from character_books JOIN characters ON character_books.character_id = characters.id GROUP BY characters.name ORDER BY total_books DESC"
end
